...
function parseSlug(slug: string): { serviceSlug: string; citySlug: string } | null {
  // Slug format: {service}-{city}-ma
  // Example: interior-painting-marlborough-ma
  if (!slug.endsWith('-ma')) {
    return null;
  }

  // Remove -ma suffix
  const withoutMa = slug.slice(0, -3);

  // Try to match against known services
  const serviceKeys = Object.keys(SERVICES);
  for (const serviceSlug of serviceKeys) {
    if (withoutMa.startsWith(serviceSlug + '-')) {
      const citySlug = withoutMa.slice(serviceSlug.length + 1);
      if (CITIES[citySlug]) {
